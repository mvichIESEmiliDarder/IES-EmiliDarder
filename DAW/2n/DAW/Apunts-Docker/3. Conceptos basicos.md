...
Un **Contenedor** es un paquete de software ligero, portátil y ejecutable que agrupa el **código de una aplicación** junto con **todas las dependencias** (librerías, archivos de configuración y binarios) necesarias para que la aplicación se ejecute de forma rápida y fiable en cualquier entorno informático.
...
Una **Imagen de Contenedor** (o simplemente "Imagen") es un **paquete inmutable** y **ejecutable** que contiene el código de una aplicación, las librerías, las dependencias, el entorno y los archivos de configuración necesarios para que la aplicación se ejecute. Es la "plantilla" de la cual se crea un contenedor.
...
El **Dockerfile** es un archivo de texto plano que contiene todas las **instrucciones** y comandos necesarios para que Docker pueda **construir automáticamente una imagen de contenedor**.
...
**Docker Hub** es un **servicio de registro (registry)** basado en la nube operado por Docker, Inc. Funciona como el **repositorio central más grande del mundo** para imágenes de contenedores Docker. 

Sería como el **"GitHub de las imágenes de contenedores"**. Su función principal es permitir a los usuarios **almacenar, compartir y descubrir** imágenes de contenedores de forma pública o privada.
